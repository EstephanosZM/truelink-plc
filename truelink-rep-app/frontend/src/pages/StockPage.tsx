...
export default function StockPage() {
  ...
  const [reqRows,     setReqRows]     = useState([{ productId: '', qty: 0 }])
  const [routeDate,   setRouteDate]   = useState(getDateOptions()[0].value)
  const [reqNotes,    setReqNotes]    = useState('')
  const [submitting,  setSubmitting]  = useState(false)
  const [reqMsg,      setReqMsg]      = useState('')
  const [history,     setHistory]     = useState<{
    id: string; products: { name: string }; quantity_requested: number
    quantity_approved: number | null; status: string; route_date: string; manager_notes: string | null
  }[]>([])
  ...
  const submitRequest = async () => {
    if (!activeRep) return
    const valid = reqRows.filter((r) => r.productId && r.qty > 0)
    if (!valid.length) return setReqMsg('Add at least one product with a quantity.')
    setSubmitting(true)

    for (const row of valid) {
      await supabase.from('stock_requests').insert({
        sales_rep_id: activeRep.id, product_id: row.productId,
        quantity_requested: row.qty, route_date: routeDate,
  ...
  const statusBadge = (status: string) => {
    ...
    return <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cfg[status] || 'bg-slate-700 text-slate-400'}`}>{status}</span>
  }

  return (
    <div className={`flex-1 overflow-y-auto pb-24 ${bg}`}>
      {/* Tab bar */}
      <div className={`flex border-b ${darkMode ? 'border-slate-800' : 'border-slate-200'} px-4 pt-4 gap-2`}>
        {[
          { key: 'stock',   label: 'My Stock'   },
          { key: 'request', label: 'Request'     },
          { key: 'history', label: 'Requests'    },
        ].map((t) => (
          <button key={t.key} onClick={() => setView(t.key as typeof view)}
            className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-colors ${
              view === t.key
                ? 'bg-blue-600 text-white'
                : `${sub} hover:text-slate-300`
            }`}>
            {t.label}
          </button>
        ))}
      ...
      <div className="px-4 pt-5 space-y-4">
                ...
                <div key={sl.id} className={`${card} border rounded-2xl p-4`}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {prod.sku_code && <p className={`${sub} text-xs`}>{prod.sku_code}</p>}
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`flex-1 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'} rounded-full h-1.5`}>
                      <div className={`h-1.5 rounded-full ${pct >= 80 ? 'bg-green-500' : pct >= 40 ? 'bg-blue-500' : 'bg-amber-500'}`}
                        style={{ width: `${pct}%` }} />
                    </div>
        ...
        {view === 'request' && (
          <div className="space-y-4">
            <div className={`${card} border rounded-2xl p-5`}>
              <p className={`${text} font-semibold mb-1`}>Request Stock</p>
              <p className={`${sub} text-xs mb-4`}>Your manager will review and approve your request.</p>
                  ...
                  {dateOptions.map((d) => (
                    <label key={d.value}
                      ...
                      <input type="radio" value={d.value} checked={routeDate === d.value}
                        onChange={() => setRouteDate(d.value)} className="accent-blue-600" />
              ...
              <div className="space-y-3 mb-4">
                {reqRows.map((row, i) => (
                  <div key={i} className={`${darkMode ? 'bg-slate-700/50' : 'bg-slate-100'} rounded-xl p-3 space-y-2`}>
                    <select value={row.productId}
                      onChange={(e) => setReqRows(reqRows.map((r, idx) => idx === i ? { ...r, productId: e.target.value } : r))}
                      className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${inp}`}>
                      <option value="">Select product…</option>
                      {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                    <div className="flex items-center gap-3">
                      <button onClick={() => setReqRows(reqRows.map((r, idx) => idx === i ? { ...r, qty: Math.max(0, r.qty - 1) } : r))}
                        className={`w-10 h-10 rounded-full ${darkMode ? 'bg-slate-600' : 'bg-slate-300'} ${text} font-bold text-xl active:scale-90`}>−</button>
                      <span className={`${text} font-bold text-xl flex-1 text-center`}>{row.qty}</span>
                      <button onClick={() => setReqRows(reqRows.map((r, idx) => idx === i ? { ...r, qty: r.qty + 1 } : r))}
                        className="w-10 h-10 rounded-full bg-blue-600 text-white font-bold text-xl active:scale-90">+</button>
                      {reqRows.length > 1 && (
                        <button onClick={() => setReqRows(reqRows.filter((_, idx) => idx !== i))}
              ...
              <button onClick={() => setReqRows([...reqRows, { productId: '', qty: 0 }])}
                className="text-blue-400 text-sm font-medium mb-4 block">+ Add another product</button>

              <textarea value={reqNotes} onChange={(e) => setReqNotes(e.target.value)}
                placeholder="Notes to manager (optional)…" rows={2}
                className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none mb-4 ${inp}`} />

              {reqMsg && <p className="text-green-400 text-sm mb-3">{reqMsg}</p>}
              ...
              <div key={req.id} className={`${card} border rounded-2xl p-4`}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className={`${sub} text-xs mt-0.5`}>
                      For {new Date(req.route_date).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-4 text-sm">
                  <div>
                    ...
                    <p className={`${text} font-medium`}>{req.quantity_requested} units</p>
                  </div>
                  {req.quantity_approved !== null && (
                    <div>
                      <p className={`${sub} text-xs`}>Approved</p>
                      <p className={`text-green-400 font-medium`}>{req.quantity_approved} units</p>
                    </div>
                  )}
                </div>
                {req.manager_notes && (
                  <p className={`${sub} text-xs mt-2`}>Manager: {req.manager_notes}</p>
                )}
              </div>
            ))}
          </div>
