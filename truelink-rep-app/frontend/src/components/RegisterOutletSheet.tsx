...
export default function OutletSheet({ onClose }: Props) {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col justify-end">
      ...
      <div className={`relative ${bg} rounded-t-2xl border-t border-slate-700 max-h-[92vh] flex flex-col`}>
            ...
            <div className="px-5 pb-8">
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`${sub} text-xs mt-1`}>Stop #{stop.sequence}</p>
                </div>
              ...
              {stop.visit && stop.visit.visit_status !== 'not_visited' && (
                <div className={`rounded-xl px-3 py-2 mb-4 text-sm font-medium ${
                  ...
                  {stop.visit.visit_status === 'sold'    && `✅ Sold — ${fmtETB((stop.sales||[]).reduce((a,r)=>a+r.total_price,0))}`}
              ...
              <div className="mb-5">
                ...
                {proxState === 'ok' && (
                  <div className="bg-green-900/30 border border-green-800 rounded-xl px-3 py-2 text-green-400 text-sm">
                    ✅ You are {proxDist}m away — within {proximityRadius}m radius
                  </div>
                )}
                {proxState === 'flagged' && (
                ...
                {proxState === 'blocked' && (
                  <div className="bg-amber-900/30 border border-amber-800 rounded-xl p-3 space-y-2">
                    <p className="text-amber-400 text-sm font-medium">
                      ⚠ You are {proxDist}m away
                    </p>
                    <p className={`${sub} text-xs`}>
                      Required: within {proximityRadius}m. Move closer to the outlet and try again.
                    </p>
                    {userCoords && (
                      <p className={`${sub} text-xs`}>
                        Outlet: {outlet.latitude.toFixed(4)}, {outlet.longitude.toFixed(4)}
                      </p>
                    )}
                  ...
                  <div className="space-y-3">
                    ...
                    {['Customer not available','Outlet closed','Road blocked','Wrong address in system'].map((r) => (
                      <label key={r} className={`flex items-center gap-3 p-3 ${card} border rounded-xl cursor-pointer`}>
                        <input type="radio" value={r} checked={flagReason===r}
                          onChange={() => setFlagReason(r)} className="accent-blue-600" />
                        <span className={`${text} text-sm`}>{r}</span>
                      </label>
                    ))}
                    <div className="flex gap-2">
                      <button onClick={() => setProxState('blocked')}
                        className={`flex-1 py-2 border border-slate-600 ${sub} rounded-lg text-sm`}>Back</button>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setView('nosale')}
                      className={`py-3 ${card} border hover:opacity-80 active:scale-95 rounded-2xl ${text} font-medium transition-all`}>
                      ✗ No Sale
                    </button>
                    <a href={`https://maps.google.com/?daddr=${outlet.latitude},${outlet.longitude}`}
                      target="_blank" rel="noreferrer"
                      className={`py-3 ${card} border hover:opacity-80 rounded-2xl ${text} font-medium transition-all flex items-center justify-center`}>
                      🗺 Navigate
                    </a>
                  </div>
                </div>
                  ...
                  <div key={item.product.id}
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className={`${sub} text-xs`}>{fmtETB(item.product.unit_price)} · {item.stock} left</p>
                      </div>
                      {item.quantity > 0 && (
                    ...
                    <div className="flex items-center gap-4">
                      <button onClick={() => updateQty(i, -1)} disabled={item.quantity === 0}
                        className={`w-11 h-11 rounded-full ${darkMode ? 'bg-slate-700' : 'bg-slate-200'} ${text} font-bold text-xl disabled:opacity-30 active:scale-90 transition-transform`}>−</button>
                      ...
                      <button onClick={() => updateQty(i, 1)} disabled={item.quantity >= item.stock || item.stock === 0}
                        className="w-11 h-11 rounded-full bg-blue-600 text-white font-bold text-xl disabled:opacity-30 active:scale-90 transition-transform">+</button>
                    </div>
                  </div>
                ))}
              </div>
              <div className="sticky bottom-0 pt-4 pb-2" style={{ background: darkMode ? '#0f172a' : '#fff' }}>
                {cartHasItems && (
                  <div className={`${card} border rounded-2xl p-4 mb-3`}>
                    <p className={`${sub} text-xs font-semibold uppercase tracking-wide mb-2`}>Cart</p>
                    {cart.filter((i) => i.quantity > 0).map((item) => (
                      <div key={item.product.id} className="flex justify-between text-sm mb-1">
                        ...
                        <span className={`${text} font-medium`}>{fmtETB(item.quantity * item.product.unit_price)}</span>
                      </div>
                    ))}
                ...
                <button onClick={completeSale} disabled={!cartHasItems || saving}
          ...
          {view === 'nosale' && (
            <div className="px-5 pb-8">
              ...
              <textarea value={noSaleNotes} onChange={(e) => setNoSaleNotes(e.target.value)}
                ...
                className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 resize-none mb-5 ${inp}`} />
