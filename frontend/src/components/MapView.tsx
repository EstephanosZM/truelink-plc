...
export default function MapView() {
  ...
  const drawPolygon = useCallback(async () => {
    ...
    // Try saved polygon — use maybeSingle() to avoid 406 when no row exists
    if (routePlan?.id) {
      ...
      if (data?.geojson) {
        ...
        const dayName = dr ? (useStore.getState() as { dayNames?: Record<number, string> }).dayNames?.[activeDay] || `Day ${activeDay}` : `Day ${activeDay}`
        L.geoJSON(data.geojson as GeoJSON.Feature, {
          ...
          .bindTooltip(`${dayName} Territory`, { sticky: true })
          .addTo(polyLayer)
        return
      }
    }

    // Fallback: convex hull of stops
    const dr = dayRoutes.find((d) => d.day === activeDay)
  ...
  const drawRoad = useCallback(async () => {
    ...
    const ph = L.polyline(waypts.map((w): L.LatLngTuple => [w.lat, w.lon]), {
      color, weight: 2, opacity: 0.2, dashArray: '6,4',
    }).addTo(roadLayer)

    const pts = await getRoadPolyline(waypts)
    ph.remove()
    L.polyline(pts, { color, weight: 4, opacity: 0.9 }).addTo(roadLayer)
  ...
  useEffect(() => {
    ...
    dayRoutes.forEach((dr) => {
      ...
      dr.stops.forEach((stop) => {
        ...
        const dayName = (useStore.getState() as { dayNames?: Record<number, string> }).dayNames?.[dr.day] || `Day ${dr.day}`
        L.marker([stop.lat, stop.lon], {
          ...
          .bindPopup(`
            <div style="min-width:160px">
              ...
              <div style="color:#64748b;font-size:12px;margin-top:4px">${dayName} · Stop #${stop.sequence}${repName ? ` · ${repName}` : ''}</div>
  ...
  useEffect(() => {
    if (activeDay !== drawnRoadDayRef.current) drawRoad()
    drawPolygon()
  }, [activeDay, drawRoad, drawPolygon])
